...
if( isset($_POST['action']) && $_POST['action'] == 'show_booking'){
    $class_room_id = $_POST['room_id'];
    $db->select('tbl_class_room_booking_details',"*",null," class_room_id = '$class_room_id'",null,null);
     $res = $db->getResult();
    // echo 123;
   // print_r($res);
   // $data = [];
...
//exit;
    ...
    (NULL, '$program_id', '$trng_type', '0', '$room_id', '".$title."',  '".$event_start_date."',  '".$event_end_date."', '1') ";
...
function chkRoomAvalibility($roomId,$startDt,$endDt){
    $db = new Database();
     $chksql = "SELECT id
     FROM `tbl_class_room_booking_details`
     WHERE `class_room_id` = $roomId
         AND ('".$startDt."' BETWEEN `from_dt` AND `to_dt`
