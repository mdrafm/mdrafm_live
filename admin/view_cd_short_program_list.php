...
<body class="user-profile">
                        ...
                        <div class="card">
                           ...
                           // echo $faculty_id;
                            ...
                            foreach($roll_id as $roll){
                               ...
                               $count = 0;
                              // $db->select('tbl_short_program_master',"s.*,c.faculty_id"," s JOIN `tbl_course_director` c ON s.course_director=c.id","s.status != 'draft' and s.course_director = '".$faculty_id."' ","s.start_date DESC",null );
                               $db->select_sql("SELECT s.id,s.course_director_id,s.prg_name,s.start_date,s.end_date,s.cert_status,s.trng_type,d.course_director,                  d.program_id FROM `tbl_short_program_master` s 
                                                     JOIN `tbl_program_directors` d ON s.course_director_id = d.id 
                                                    JOIN `tbl_faculty_master` f ON d.course_director = f.id OR d.asst_course_director = f.id
                                                    WHERE f.user_id ='".$_SESSION['user_id']."'order by s.id desc");
                                                          ...
                                                          if ($row['crt_status']==0){
                                                            ?>  
                                                               <input type="button" class="m-2"  style="background:#3292a2;border: 0;
                                                                                    ...
                                                                                    border-radius: 3px;
                                                                                    color: #fff;"
                                                        name="send" onclick="datapost('approve_crt.php',{id: <?php echo $row['id'] ?> ,trng_type:<?php echo $row['trng_type'] ?>})" value="Approve Crt" />
                                                            <?php 
                                                              }
                                                            ?> 
                                                         <?php
 ...
 <div class="modal fade" id="crtModal" tabindex="-1" aria-labelledby="termModalLabel"  aria-hidden="true">

    <div class="modal-dialog">
        <div class="modal-content"
            style="width:200%; margin:20px -100px">
            <div class="modal-header">
                <h5 class="modal-title" id="termModalLabel">Certificate Approval</h5>
           ...
           <div class="modal-body">
               <?php //include "download_crt_trainee.php" ?>
           </div>
            <div class="modal-footer justify-content-between">
...
function add_course_co(id,co_officer_id){
    ...
    function ViewModalCrt(id,trng_type){
        console.log(123);
        $('#crtModal').modal('show');
    }
