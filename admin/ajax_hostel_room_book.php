...
if (isset($_POST['action']) && $_POST['action'] == 'store_room') {

    $insert_sql = "INSERT INTO `tbl_hostel_room_booking` (`id`, `room_id`, `bed_id`, `trng_type`, `program_id`, `trainee_id`, `cadre`, `check_in`, `check_out`, `remark`, `status`) 
    ...
    $db->update("tbl_hostel_bed_master", ['booking_id' => $last_insert_id,'status' => 1], 'id=' . $_POST['bedId']);
    $res1 = $db->getResult();
     print_r($res1);
        ...
        <form action="" id="roomBookFrm">
            ...
            <div class="row">
                ...
                <input type="button" class="btn" style="background:#3292a2;margin-left: 12vw;" id="saveBtn" name="send"
                    onclick="bookRoom()" value="Book">
            </div>
        </form>
        <?php
    } else {

        $bookin_sql = "SELECT b.id,b.check_in,CONCAT(n.f_name,' ',n.l_name)as name,p.prg_name,n.sex,n.phone,n.email FROM `tbl_hostel_room_booking` b 
...
if (isset($_POST['action']) && $_POST['action'] == 'checkOut_booking') {
    ...
    $db->update('tbl_hostel_room_booking', ["check_out" => $chk_date, 'status' => 0], 'id=' . $bookingId);
    $res = $db->getResult();
    if ($res) {
        // echo 123;
        $response['status'] = 1;
