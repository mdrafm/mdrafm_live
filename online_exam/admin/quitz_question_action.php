...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		$main_query = "
		  SELECT s.*,p.paper_title  FROM `surprise_test_question` s 
		  JOIN `tbl_quitz_paper_master` p ON s.paper_id = p.id
		  WHERE s.`syllabus_id` = 7 AND s.`paper_id` = ".$_POST["paper_id"];
