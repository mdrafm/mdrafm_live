...
if (isset($_POST["action"])) {
    if ($_POST["action"] == 'fetch') {
        ...
        $main_query = "
         ...
         SELECT  m.id,m.program_id,m.exam_title,m.total_question,m.set_question_paper,m.exam_category,m.paper_id, f.name as faculty_name, 
         ...
         WHERE   m.exam_category = 5 AND  f.phone = " . $_SESSION['username'];
        ...
        foreach ($result as $row) {
            ...
            if ($row['status'] == 4) {
                ...
                <div  align="center">
                 <button type="button" id="set_qstn" class="btn btn-success  btn-sm" 
                   data-id="' . $row["id"] . '"
                    onclick = "setQuestion(' . $exam_id . ',' . $program_id . ',' . $paper_id . ')" >Set Qustion</button>
                ...
                <div  align="center">
                 <button type="button" id="set_qstn" class="btn btn-success  btn-sm" 
                   data-id="' . $row["id"] . '"
                    onclick = "takeAttendance(' . $exam_id . ',' . $program_id . ',' . $paper_id . ')" >Take Attendance</button>
                ...
                <button type="button" class="btn btn-success  btn-sm" data-id="' . $row["id"] . '"
                 onclick = "setQuestionInd(' . $exam_id . ',' . $program_id . ',' . $paper_id . ')" >Set Question Ind</button>
            ...
            if ($row['status'] == 6) {
                ...
                <div  align="center">
                <button type="button" id="set_qstn" class="btn btn-success  btn-sm" 
                onclick = "view_result(' . $exam_id . ','.$exam_category.','.$paper_id.')" >View Result</button>
...
//exit;
            ...
            FROM `surprise_test_question` WHERE syllabus_id = 7 AND paper_id = '" .$paper_id. "'
            ORDER BY rand() ";
