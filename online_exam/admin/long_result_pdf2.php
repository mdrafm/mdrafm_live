...
$object_sql = "
SELECT m.exam_title,m.exam_date_time,m.exam_duration,m.total_question,m.marks_per_right_answer,p.prg_name,CONCAT(a.paper_code, ' ',a.title) as paper FROM `tbl_exam_master` m 
...
$obj_pdf->writeHTML($title,true, false, true, false, '');
 ...
 $exam_date_time = '<p style="font-size:18px;">Date & Time: '.$exam_time.'</p>';
 $obj_pdf->Ln(5);
 $obj_pdf->writeHTML($exam_date_time,true, false, true, false, '');

 $exam_duration = '<p style="font-size:18px;">Exam Duration: '.$duration.'</p>';
 $obj_pdf->Ln(5);
 $obj_pdf->writeHTML($exam_duration,true, false, true, false, '');

 $total_question = '<p style="font-size:18px;">Total Question: '.$exam_row['total_question'].'</p>';
 $obj_pdf->Ln(5);
 $obj_pdf->writeHTML($total_question,true, false, true, false, '');

 $total_mark = '<p style="font-size:18px;">Total Mark: '.$total_mark.'</p>';
 $obj_pdf->Ln(5);
 $obj_pdf->writeHTML($total_mark,true, false, true, false, '');
