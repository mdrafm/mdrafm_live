...
if(isset($_SESSION['exam_id'])){
    ...
    ";
     ...
     $object->query = "SELECT exam_category,trng_type FROM `tbl_exam_master`  WHERE id =  '".$_SESSION['exam_id']."'  ";
     $cat_result = $object->get_result();

        foreach($cat_result as $cat_row)
        {
            $exam_category = $cat_row['exam_category'];
            $trng_type = $cat_row['trng_type'];
        }
    ...
    if($exam_category ==1){
        ...
        m.marks_per_right_answer,m.marks_per_wrong_answer,m.status,i.id as trainee_info_id,i.exam_date_time,i.exam_duration,d.name 
        ...
        JOIN `tbl_user` d ON i.trainee_id = d.id
   ...
   if($exam_category ==2){
    ...
    JOIN `tbl_trainee_exam_info` i ON m.id = i.exam_id
    JOIN `tbl_dept_trainee_registration` d ON d.user_id= i.trainee_id
   ...
   if($exam_category ==3){
       ...
       if($trng_type == 1 ){
           $program = 'tbl_program_master';
       }elseif($trng_type == 3){
           $program = 'tbl_mid_program_master';
       }elseif($trng_type == 4){
           $program = 'tbl_short_program_master';
       }


    $object->query = "
        SELECT m.id,m.exam_title,pm.prg_name,m.total_question,
        m.marks_per_right_answer,m.marks_per_wrong_answer,m.status,i.id as trainee_info_id,i.exam_date_time,i.exam_duration,d.name 
        FROM `tbl_exam_master` m 
        JOIN `".$program ."` pm ON m.program_id = pm.id
        JOIN `tbl_trainee_exam_info` i ON m.id = i.exam_id
        JOIN `tbl_user` d ON i.trainee_id = d.id
        WHERE m.id =  '".$_SESSION['exam_id']."' AND i.trainee_id =  '".$_SESSION['user_id']."' ";
...
<div class="card shadow mb-4" id="exam-wrap" >
            ...
            <div class="col-md-6 d-flex justify-content-between">
                <b>Paper : </b><?php echo $paper; //$object->Get_Subject_name($subject_id); ?>
                 <div class="mobileView" style="display:none">
                    <div class="exam_timer" data-timer="<?php echo $remaining_minutes; ?>" style="max-width:20rem; width: 100%; height: 12vh; margin: -2rem auto"></div>
                </div>    
            </div>
        </div>
    </div>
    <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-md-4" id="webView">
                <!-- timer div -->
                
                <div class="row">
                <div class="info">
                <div class="col-md-6">
                        <div class="exam_timer" data-timer="<?php echo $remaining_minutes; ?>"
                            style="max-width:20rem; width: 100%; height: 25vh; margin:0 auto"></div>
                    </div>
                <div class="col-md-12">
...
$(document).ready(function() {
    ...
    var exam_category = "<?php echo $exam_category ?>";
    var action = '';
    var save_ans_action = '';
        if(exam_category==3){
             action = 'load_surprise_question';
             save_ans_action = 'surprise_answer'
        }else{
             action = 'load_question';
             save_ans_action = 'answer'
        }
console.log(action);
