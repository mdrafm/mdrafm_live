...
// $object_sql = "
//  SELECT m.exam_title,m.exam_title,m.exam_date_time,m.exam_duration,m.total_question,m.marks_per_right_answer,p.prg_name,a.paper_code as paper FROM `tbl_exam_master` m 
//  JOIN `tbl_mid_program_master` p ON m.program_id = p.id
//  JOIN `tbl_mid_paper_master` a ON m.paper_id = a.id
//  WHERE m.id = ".$post_exam_id;

$object_sql = "
 SELECT m.exam_title,m.exam_title,m.exam_date_time,m.exam_duration,m.total_question,m.marks_per_right_answer,p.prg_name FROM `tbl_exam_master` m 
...
// WHERE tbl_exm_inf.exam_status = 1 AND tbl_exm_inf.exam_id ='".$post_exam_id."' AND tbl_trainee_info.trng_type =3  AND tbl_trainee_info.program_id = 68
// group by tbl_qs_ans.trainee_exam_info_id";

 $result_sql = "SELECT tbl_exm_inf.id,tbl_trainee_info.user_id,tbl_trainee_info.name,tbl_trainee_info.phone,sum(tbl_qs_ans.marks) as tot_mark 
                            FROM `tbl_dept_trainee_registration` tbl_trainee_info
                            join tbl_trainee_exam_info tbl_exm_inf on tbl_exm_inf.trainee_id = tbl_trainee_info.user_id 
                            join tbl_exam_question_answer as tbl_qs_ans on  tbl_qs_ans.trainee_exam_info_id=tbl_exm_inf.id
...
$count++;
   ...
   $res .= '<tr>
        ...
        <td style="border: 1px solid black; width:18%;font-size:13px;text-align: center;" >'.$remark.'</td>
        </tr>';
...
$suggestion =  '<table align="left" width="100%" style="text-align:left;border: 1px solid black;">
                ...
                <tr style="border: 1px solid black;" >
                    ...
                    <th  style="border: 1px solid black; width:18%;font-size:15px;font-weight:bold">Remark</th>
