 ...
 //print_r($row);
  switch ($row['sex']) {
   ...
   //echo $row['sex'];
    ...
    $crt_no = '<p style="font-size:18px;">Crt No :'.$row['crt_no'].' </p>';
  ...
  // $obj_pdf->SetTextColor(0, 0, 0);
    ...
    // $obj_pdf->Ln(5);
     $para1 = '
               ...
               <p style="font-size:19px;" align="center"><b>'.$prefix.' '.$row['name'].' </b> </p>
               <p style="font-size:19px;" align="center">'.$row['designation'].', '.$row['office_name'].'</p>
               <p style="font-size:19px;" align="center"> has participated in the training program of   </p>
               <p style="font-size:19px;" align="center">“'.$row['prg_name'].'”  </p>
               <p style="font-size:19px;" align="center">from '.date("d-m-Y", strtotime($row['start_date'])).' to '. date("d-m-Y", strtotime($row['end_date'])).'</p>
     ';
     //$obj_pdf->Ln(5);
...
// Add the signature image
$obj_pdf->Image($course_director_sign, $signature_x, $signature_y, 15, 25, '', '', '', false, 500, '', false, false, 0, false, false, false);
...
//QRCODE,L : QR-CODE Low error correction
 ...
 $txt = "Crt No : ".$row['crt_no']."\n"."Name : ".$row['name']."\n"."Designation : ".$row['designation']."\n"."Program Name : ".$row['prg_name']."\n"."From : ".$row['start_date']." To ".$row['end_date'];
$obj_pdf->write2DBarcode($txt , 'QRCODE,R', 20, 135, 30, 30, $style, 'N');
