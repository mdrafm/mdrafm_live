...
if(isset($_POST["action"]))
{
   ...
   //       WHERE   f.phone = ".$_SESSION['username'] ;
         ...
         SELECT  m.id,m.program_id,m.exam_title,m.total_question,m.set_question_paper,m.exam_category, f.name as faculty_name, p.prg_name as program_name, 
     ...
     //print_r($result);
		...
		foreach($result as $row)
		{
            ...
            if($row['status'] == 6 ){
                ...
                <div  align="center">
                <button type="button" id="set_qstn" class="btn btn-success  btn-sm" 
                onclick = "view_result('.$exam_id.','.$exam_category.','.$program_id.')">View Result</button>
    ...
    if($_POST["action"] == 'set_question')
	{
        ...
        if($object->row_count() > 0)   {
            ...
            foreach ($res_qstn as $row_qstn) {
                ...
                VALUES (:exam_question_id,:exam_id)
                ";
        ...
        $object->query = "
        SELECT t.user_id,m.id as exam_id,m.exam_date_time,m.exam_duration,m.total_question,m.status  FROM `exam_trainee_list`  t
        JOIN `tbl_exam_master` m ON t.program_id = m.program_id
        WHERE m.id = '".$_POST['exam_id']."' AND t.program_id = '".$_POST['program_id']."' AND t.trng_type = 3" ;
        ...
        ?>
         <table class="table table-bordered" id="trainne_attn_table" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    $object->query = "
                        SELECT e.*,i.name as trainee_name,i.phone,e.attandance FROM `tbl_trainee_exam_info` e 
                        JOIN `tbl_dept_trainee_registration` i ON e.trainee_id = i.user_id
                        WHERE e.exam_id = $exam_id  AND i.program_id = 13 and trng_type = 3
